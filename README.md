...
보수적인 시작 권장치는 매장 10개이며 안정화 뒤 20~30개를 검토합니다. 매장별 동시 활성 주문은 최대 100개, 초기 동시 사용자 검증은 50명, 일일 고객 방문은 1,000~3,000명, 일일 주문은 300~1,000건을 안전 목표로 둡니다. 이 수치는 보장값이나 Cloudflare 한도가 아닙니다.

Cloudflare 대시보드에서 Workers 요청량과 D1 행 읽기·쓰기를 각각 한도의 50%, 70%, 85% 지점에서 확인합니다. 50%에서는 API별 호출 수와 쿼리 행 수를 기록하고, 70%에서는 조회 간격·페이지 크기·인덱스를 재검토하며, 85%에서는 신규 매장 확대를 중단하고 트래픽 또는 데이터 보관 정책을 조정합니다. D1 저장공간이 250MB에 도달하면 확장 또는 정산·법적 보존 요구를 반영한 별도 보관 정책을 검토합니다.

재산정할 때는 브라우저 개발자 도구와 Workers Metrics에서 API별 실제 요청 수, D1 Metrics에서 rows read/written, 쿼리 시간과 저장공간을 같은 7일 기간으로 측정합니다. `매장 수 × 활성 시간 × 실제 polling 횟수`와 고객 방문·주문당 관측 호출/행 수를 곱해 일일치를 다시 계산하고, 피크 시간대 50명 검증 결과와 함께 기록합니다. 판매자 주문 조회는 기본 15~30초, 숨김 탭 중지, 표시 직후 1회 조회, 오류 시 최대 120초 지수 재시도를 사용합니다.
...
공개 정보 페이지는 `/about`, `/guide`, `/privacy`, `/terms`입니다. 자동 광고 허용 대상은 로그인 없이 방문하는 공개 소비자 메뉴 `/shop/<매장 slug>`뿐입니다. `/`, `/login`, `/seller`, `/about`, `/guide`, `/privacy`, `/terms`, `/api/*`는 AdSense 페이지 제외로 관리하며 수동 광고 슬롯은 만들지 않습니다.

`public/ads.txt`, AdSense 계정 확인용 메타 태그와 공식 자동 광고 스크립트에는 게시자 `ca-pub-4934943702995460`이 반영되어 있습니다. 별도 `<ins class="adsbygoogle">` 슬롯과 임의 슬롯 ID는 사용하지 않습니다. 광고 노출 범위는 AdSense 대시보드의 페이지 제외가 우선하므로 Production 적용 전에 위 제외 경로를 반드시 등록해야 합니다.
