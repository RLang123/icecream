...
test("판매자는 진행 중 주문의 메뉴·사이즈·수량·금액을 수정하고 합계를 다시 계산한다", async () => {
  let updated;
  const store={store:{name:'매장',ingredients:[{id:'ingredient-1',name:'재료',available:true,stock:10}]},categories:['전체'],items:[{id:'snack',name:'과자',price:1500,smallPrice:1500,largePrice:2000,sizesEnabled:true,temperatureMode:'none',ingredientIds:['ingredient-1']}]};
  const previous=[{id:'snack',name:'과자',temperature:'NONE',size:'S',shots:0,shotPrice:500,price:1500,qty:1,ingredientIds:['ingredient-1']}];
  const env={DB:{prepare(sql){let args=[];return{sql,bind(...values){args=values;this.args=args;return this;},async first(){if(sql.includes('FROM sessions JOIN users'))return{id:'seller-1',email:'seller@test.invalid',name:'판매자',role:'seller'};if(sql.startsWith('SELECT status,items,total'))return{status:'new',items:JSON.stringify(previous),total:1500,details_cleaned_at:null};if(sql.includes('FROM order_change_requests'))return null;if(sql.startsWith('SELECT data,inventory_version'))return{data:JSON.stringify(store),inventory_version:3};throw new Error(sql);}};},async batch(statements){const orderStatement=statements.find(statement=>statement.sql.startsWith('UPDATE orders SET items='));updated=orderStatement.args;return statements.map(()=>({meta:{changes:1}}));}}};
  const items=[{id:'snack',name:'과자',emoji:'🍪',temperature:'NONE',size:'S',shots:0,price:1500,qty:2}];
  const response=await api(new Request('https://example.com/api/orders/order-edit-1',{method:'PATCH',headers:{cookie:'session=edit-order','content-type':'application/json'},body:JSON.stringify({action:'edit',items,requestKey:'edit-request-123456'})}),env,{});
