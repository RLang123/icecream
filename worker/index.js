...
export async function api(request, env, ctx) {
  ...
  if(path.startsWith('/api/orders/') && method==='PATCH') {
    ...
    if(input.action==='undo'){
      ...
      let result;if(change.change_type==='items')result=await env.DB.prepare("UPDATE orders SET items=?,total=? WHERE id=? AND seller_id=? AND status IN ('new','preparing')").bind(change.before_items,change.before_total,id,user.id).run();else result=await env.DB.prepare("UPDATE orders SET status=?,completed_at=CASE WHEN ?='completed' THEN completed_at ELSE NULL END,payment_method=CASE WHEN ?='completed' THEN payment_method ELSE NULL END WHERE id=? AND seller_id=?").bind(change.before_status,change.before_status,change.before_status,id,user.id).run();if(!result.meta.changes)return json({error:'현재 주문 상태에서는 되돌릴 수 없습니다.'},409);await env.DB.prepare('UPDATE order_changes SET undone_at=CURRENT_TIMESTAMP WHERE id=? AND seller_id=?').bind(change.id,user.id).run();return json({ok:true});
    }
    if(input.action==='edit'){
      if(!['new','preparing'].includes(order.status)||order.details_cleaned_at)return json({error:'대기 또는 준비 중인 주문만 수정할 수 있습니다.'},409);
      const requested=Array.isArray(input.items)?input.items:[];if(!requested.length||requested.length>30)return json({error:'주문 품목을 확인해 주세요.'},400);
      const items=[];let total=0;for(const line of requested){const qty=Math.floor(Number(line.qty));const price=Math.round(Number(line.price));const size=String(line.size||'NONE');if(qty<1||qty>20||!Number.isFinite(price)||price<0||price>10000000||!['S','L','NONE'].includes(size))return json({error:'수량, 사이즈 또는 금액을 확인해 주세요.'},400);const safe={id:String(line.id||'').slice(0,80),name:String(line.name||'').trim().slice(0,80),emoji:String(line.emoji||'').slice(0,8),temperature:String(line.temperature||'NONE').slice(0,10),size,shots:Math.max(0,Math.min(100,Math.floor(Number(line.shots||0)))),shotPrice:Math.max(0,Math.round(Number(line.shotPrice||0))),price,qty};if(!safe.id||!safe.name)return json({error:'메뉴 정보를 확인해 주세요.'},400);items.push(safe);total+=price*qty;}
      if(total>10000000)return json({error:'주문 합계 금액을 확인해 주세요.'},400);let previousItems=[];try{previousItems=JSON.parse(order.items)}catch{}const details=items.flatMap((item,index)=>{const before=previousItems[index];if(!before)return [`${item.name} 추가`];const changes=[];if(before.name!==item.name)changes.push(`${before.name} → ${item.name}`);if(before.size!==item.size)changes.push(`사이즈 ${before.size} → ${item.size}`);if(Number(before.qty)!==item.qty)changes.push(`수량 ${before.qty} → ${item.qty}`);return changes});details.push(`금액 ${Number(order.total).toLocaleString('ko-KR')}원 → ${total.toLocaleString('ko-KR')}원`);const summary=details.join(' · ').slice(0,300);const afterItems=JSON.stringify(items);const result=await env.DB.prepare("UPDATE orders SET items=?,total=? WHERE id=? AND seller_id=? AND status IN ('new','preparing') AND details_cleaned_at IS NULL").bind(afterItems,total,id,user.id).run();if(!result.meta.changes)return json({error:'다른 화면에서 주문 상태가 변경되었습니다.'},409);await env.DB.prepare('INSERT INTO order_changes(id,order_id,seller_id,change_type,summary,before_items,after_items,before_total,after_total) VALUES(?,?,?,?,?,?,?,?,?)').bind(uid(),id,user.id,'items',summary,order.items,afterItems,Number(order.total),total).run();return json({ok:true,items,total,summary});
    }
