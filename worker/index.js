...
function projectError(data) {
  if (!data || typeof data !== 'object' || !data.store || !Array.isArray(data.categories) || !Array.isArray(data.items)) return '프로젝트 형식이 올바르지 않습니다.';
  if (data.categories.length > 50 || data.items.length > 500) return '카테고리 또는 메뉴가 너무 많습니다.';
  if (!String(data.store.name || '').trim()) return '매장 이름을 입력해 주세요.';
  if (data.store.ingredients !== undefined && !Array.isArray(data.store.ingredients)) return '재료 목록 형식이 올바르지 않습니다.';
  ...
  for (const ingredient of data.store.ingredients || []) {
    ...
    if (!id || ingredientIds.has(id) || !name || name.length > 80 || (ingredient.available !== undefined && typeof ingredient.available !== 'boolean')) return '재료 이름, 상태 또는 식별자를 확인해 주세요.';
    ingredientIds.add(id);
  }
  const ids = new Set();
  for (const item of data.items) {
    if (item.id === undefined || ids.has(String(item.id))) return '메뉴 식별자가 중복되거나 없습니다.';
    ids.add(String(item.id));
    if (!String(item.name || '').trim() || !Number.isFinite(Number(item.price)) || Number(item.price) < 0 || Number(item.price) > 10000000) return '메뉴 이름과 가격을 확인해 주세요.';
    if (item.ingredientIds !== undefined && (!Array.isArray(item.ingredientIds) || item.ingredientIds.some(id => typeof id !== 'string' || !ingredientIds.has(id)))) return `${String(item.name || '메뉴')}의 재료 연결 정보를 확인해 주세요.`;
  }
  if (JSON.stringify(data).length > 2500000) return '사진이 너무 많아 프로젝트 저장 용량을 초과했습니다. 일부 사진을 제거해 주세요.';
  return null;
}
