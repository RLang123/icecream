...
export async function api(request, env, ctx) {
  ...
  const protectedRoute = (path==='/api/me' && method==='GET') || (path==='/api/project' && ['GET','PUT'].includes(method)) || (path==='/api/export' && method==='POST') || (path==='/api/orders' && method==='GET') || (path==='/api/my-orders' && method==='GET') || (method==='PATCH' && /^\/api\/orders\/[^/]+$/.test(path));
